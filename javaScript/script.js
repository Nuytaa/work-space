...
const getData = async (url, cbSucsess, cbError) => {
 try { 
...
<article class="vacancy" tabindex="0" data-id="${vacancy.id}">
    <img src="${API_URL}${vacancy.logo}" alt="logo company ${vacancy.company}" class="vacancy_img">

    <p class="vacancy_company">${vacancy.company}</p>

    <h3 class="vacancy_title">${vacancy.title}</h3>

    <ul class="vacancy_fields">
        <li class="vacancy_field">от ${parseInt(vacancy.salary).toLocaleString()}₽</li>
        <li class="vacancy_field">${vacancy.format}</li>
        <li class="vacancy_field">${vacancy.type}</li>
        <li class="vacancy_field">${vacancy.experience}</li>
    </ul>
</article>
`;
...
const createDetailVacancy = ({
    ...
    <article class="detail">
        <div class="detail_header">
            <img src="${API_URL}${logo}" alt="logo company ${company}" class="detail_logo">
            <p class="detail_company">${company}</p>
        ...
        <div class="detail_main">
            ...
            <ul class="detail_fields">
                <li class="detail_field">от ${parseInt(salary).toLocaleString()}₽</li>
                <li class="detail_field">${type}</li>
                <li class="detail_field">${format}</li>
                <li class="detail_field">${experience}</li>
                <li class="detail_field">${location}</li>
            </ul>
        </div>

        <p class="detail_resume">Отправляйте резюме на 
            <a class="blue_text" 
...
const init = () => {
    ...
    const vacanciesFilterBtn = document.querySelector('.vacancies_filter-btn');
    const vacanciesFilter = document.querySelector('.vacancies_filter');

    vacanciesFilterBtn.addEventListener('click', () => {
        if(vacanciesFilterBtn.classList.contains('vacancies_filter-btn_active')) {
            closeFilter(
                vacanciesFilterBtn, 
                vacanciesFilter, 
